...
$$
DHI = .rac{2 .imes .ext{Height}_{.ext{disc}}}{.ext{Height}_{.ext{upper}} + .ext{Height}_{.ext{lower}}}
$$
...
$$
ASI = .rac{|.ext{SI}_{.ext{peak.NP}} - .ext{SI}_{.ext{peak.AF}}|}{.ext{SI}_{.ext{mean.CSF}}}
$$
*   **$.ext{SI}_{.ext{peak.NP}}$ 和 $.ext{SI}_{.ext{peak.AF}}$**:
    *   首先从图像的椎间盘感兴趣区域中提取所有像素的信号强度值。
    *   然后，利用高斯混合模型对这些信号强度值进行建模。高斯混合模型假设感兴趣区域内的像素信号强度由两个独立的、遵循高斯分布的组（即髓核和纤维环）混合而成。
    *   模型通过迭代优化，找到这两个高斯分布的**均值（峰值）**和**方差**。其中，信号强度较高的均值被识别为髓核峰值信号强度 ($.ext{SI}_{.ext{peak.NP}}$)，而信号强度较低的均值被识别为**纤维环峰值信号强度** ($.ext{SI}_{.ext{peak.AF}}$)。
    *   **峰值信号强度差** ($|.ext{SI}_{.ext{peak.NP}} - .ext{SI}_{.ext{peak.AF}}|$ ) 即为这两个峰值均值之间的绝对差值。
*   **$.ext{SI}_{.ext{mean.CSF}}$ (脑脊液平均信号强度)**:
...
*   **高斯混合模型算法**:
    ...
    $$
    p(x) = .um_{k=1}^{K} .i_k .athcal{N}(x | .u_k, .igma_k^2)
    $$
    其中：
    *   $K$ 是混合分量的数量（在本项目中，$K=2$，分别代表髓核和纤维环）。
    *   $.i_k$ 是第 $k$ 个高斯分量的**混合权重**，满足 $.um_{k=1}^{K} .i_k = 1$ 且 $0 .e .i_k .e 1$。它表示一个数据点来自第 $k$ 个分量的先验概率。
    *   $.athcal{N}(x | .u_k, .igma_k^2)$ 是第 $k$ 个高斯分量的概率密度函数，其均值为 $.u_k$ (即信号峰值)，方差为 $.igma_k^2$。

    为了从数据中找出这些未知参数 ($.i_k, .u_k, .igma_k^2$)，我们采用最大期望算法，通过迭代来最大化数据的对数似然函数。该算法包含以下两个核心步骤：

    1.  **期望步 (E-step)**: 在这一步，我们基于当前的参数估计，计算每个数据点 $x_n$ 由每个高斯分量 $k$ 生成的后验概率，也称为“责任”：
        $$
        .amma(z_{nk}) = .rac{.i_k .athcal{N}(x_n | .u_k, .igma_k^2)}{.um_{j=1}^{K} .i_j .athcal{N}(x_n | .u_j, .igma_j^2)}
        $$
        $.amma(z_{nk})$ 可以理解为数据点 $x_n$ “属于” 第 $k$ 个分量的概率。

    2.  **最大化步 (M-step)**: 在这一步，我们使用上一步计算出的“责任”来更新模型参数，以最大化期望的对数似然。更新公式如下：
        *   更新均值 $.u_k$：
            $$
            .u_k^{.ext{new}} = .rac{1}{N_k} .um_{n=1}^{N} .amma(z_{nk}) x_n
            $$
        *   更新方差 $.igma_k^2$：
            $$
            (.igma_k^2)^{.ext{new}} = .rac{1}{N_k} .um_{n=1}^{N} .amma(z_{nk}) (x_n - .u_k^{.ext{new}})^2
            $$
        *   更新混合权重 $.i_k$：
            $$
            .i_k^{.ext{new}} = .rac{N_k}{N}
            $$
        其中，$N$ 是数据点的总数，$N_k = .um_{n=1}^{N} .amma(z_{nk})$ 是分配给分量 $k$ 的有效数据点数。

    算法通过反复交替执行 E 步和 M 步，直到参数收敛或达到最大迭代次数。最终，得到的两个均值 $.u_1$ 和 $.u_2$ 即被认为是髓核和纤维环的峰值信号强度。
...
3.  **盒计数法算法**:
    ...
    4.  **对数变换与拟合**: 收集所有 $(s_i, N(s_i))$ 对。然后计算 $.og(s_i)$ 和 $.og(N(s_i))$。
    5.  **线性回归**: 对数据点 $(.og(s_i), .og(N(s_i)))$ 执行线性回归，得到一条最佳拟合直线。
    6.  **计算维度**: 这条直线的斜率的负值即为该图像的分形维度D。

#### T2信号强度比率 (T2SI)
根据`TARGET-ROI`策略，在髓核中最亮的区域和椎管内的脑脊液中分别定义ROI。T2SI是这两个区域平均信号强度的直接比率，用于客观评估髓核的水合状态。
$$
ASI = .rac{.ext{SI}_{.ext{mean.NP}}}{.ext{SI}_{.ext{mean.CSF}}}
$$
*   **$.ext{SI}_{.ext{mean.NP}}$ (髓核平均信号强度)**:
    ...
    *   最终，髓核平均信号强度 ($.ext{SI}_{.ext{mean.NP}}$) 是该“最亮髓核代理区域”内所有像素的平均信号强度。
*   **$.ext{SI}_{.ext{mean.CSF}}$ (脑脊液平均信号强度)**:
    *   使用统一的椎管内容物掩码（标签值在 `config.py` 中由 `DURAL_SAC_LABEL` 定义）作为信号强度参照区域。
    *   程序在计算平均值之前，会自动执行一个信号提纯步骤：它会移除该区域内信号强度最低的25%的像素（通常对应神经根等非CSF组织），仅使用剩余75%的较亮像素来计算平均信号强度。
...
1.  **构建Gabor滤波器组**: Gabor滤波器本质上是一个由高斯函数调制的复正弦函数。一个2D的Gabor滤波器可以通过以下公式定义：
    $$
    g(x, y; .ambda, .heta, .si, .igma, .amma) = .xp.eft(-.rac{x'^2 + .amma^2 y'^2}{2.igma^2}.ight) .os.eft(2.i .rac{x'}{.ambda} + .si.ight)
    $$
    其中：
    *   $x' = x .os.heta + y .in.heta$ 和 $y' = -x .in.heta + y .os.heta$ 是旋转后的坐标。
    *   $.ambda$ (波长): 正弦波的波长，决定了要捕捉的纹理尺度。程序根据预设的波长列表（`wavelengths`参数）进行选择。
    *   $.heta$ (方向): 滤波器的方向。程序根据预设的方向列表（`orientations`参数）生成多个方向的滤波器。
    *   $.si$ (相位偏移): 相位偏移。
    *   $.igma$ (高斯标准差): 高斯包络的标准差，决定了滤波器的有效范围。
    *   $.amma$ (空间纵横比): 决定了Gabor形状的椭圆度。
...
3.  **特征提取**: 最后，在每个Gabor响应图的椎间盘感兴趣区域内，计算一系列统计指标：
    ...
    *   **能量**: 响应图在该感兴趣区域内所有像素强度平方和，即 $.um I_{ROI}^2$。它量化了纹理的“活跃度”或“强度”。
...
2.  **几何矩 ($M_{pq}$)**: 对于这个二值图像，其 $(p+q)$ 阶几何矩 $M_{pq}$ 定义为图像内所有像素位置 $(x, y)$ 及其对应像素值 $I(x, y)$ 的加权和：
    $$
    M_{pq} = .um_{x}.um_{y} x^p y^q I(x, y)
    $$
    其中，$I(x, y)$ 是像素 $(x, y)$ 的值（二值图像中为0或1）。
3.  **中心矩 ($.u_{pq}$)**: 为了消除平移对特征的影响，引入中心矩。它是在图像的质心 $(.ar{x}, .ar{y})$ 处计算的：
    $$
    .u_{pq} = .um_{x}.um_{y} (x-.ar{x})^p (y-.ar{y})^q I(x, y) .uad .ext{其中 } .ar{x} = .rac{M_{10}}{M_{00}}, .ar{y} = .rac{M_{01}}{M_{00}}
    $$
    其中，$M_{00}$ 是图像的总像素数或总质量。
4.  **归一化中心矩 ($.ta_{pq}$)**: 为了进一步消除缩放对特征的影响，中心矩需要进行归一化：
    $$
    .ta_{pq} = .rac{.u_{pq}}{.u_{00}^{(1 + .rac{p+q}{2})}}
    $$
5.  **Hu不变矩 ($h_1, .ots, h_7$)**: Hu通过对这些归一化的中心矩进行七种特定的非线性组合，推导出了七个对平移、旋转和缩放都保持不变的矩不变量。这七个公式列举如下：
    $$
    h_1 = .ta_{20} + .ta_{02}
    $$
    $$
    h_2 = (.ta_{20} - .ta_{02})^2 + 4.ta_{11}^2
    $$
    $$
    h_3 = (.ta_{30} - 3.ta_{12})^2 + (3.ta_{21} - .ta_{03})^2
    $$
    $$
    h_4 = (.ta_{30} + .ta_{12})^2 + (.ta_{21} + .ta_{03})^2
    $$
    $$
    h_5 = (.ta_{30} - 3.ta_{12})(.ta_{30} + .ta_{12})[(.ta_{30} + .ta_{12})^2 - 3(.ta_{21} + .ta_{03})^2] + (3.ta_{21} - .ta_{03})(.ta_{21} + .ta_{03})[3(.ta_{30} + .ta_{12})^2 - (.ta_{21} + .ta_{03})^2]
    $$
    $$
    h_6 = (.ta_{20} - .ta_{02})[(.ta_{30} + .ta_{12})^2 - (.ta_{21} + .ta_{03})^2] + 4.ta_{11}(.ta_{30} + .ta_{12})(.ta_{21} + .ta_{03})
    $$
    $$
    h_7 = (3.ta_{21} - .ta_{03})(.ta_{30} + .ta_{12})[(.ta_{30} + .ta_{12})^2 - 3(.ta_{21} + .ta_{03})^2] - (.ta_{30} - 3.ta_{12})(.ta_{21} + .ta_{03})[3(.ta_{30} + .ta_{12})^2 - (.ta_{21} + .ta_{03})^2]
    $$
...
2.  **LBP值**: 将这个二进制数转换为十进制数，即为中心像素的LBP值。本项目采用`uniform`模式，它将某些不常见的LBP模式映射到特定的值，并为所有非`uniform`模式分配一个公共值。
    $$
    LBP_{P,R}(x_c, y_c) = .um_{p=0}^{P-1} s(g_p - g_c) 2^p .uad .ext{其中 } s(z) = .egin{cases} 1 & z .eq 0 . 0 & z < 0 .nd{cases}
    $$
3.  **LBP特征**:
...
2.  **骨架分析**
    ...
    *   **提取的特征**:
        ...
        $$
        C(p) = .eft( .um_{i=-1}^{1} .um_{j=-1}^{1} S(p_x+i, p_y+j) .ight) - 1
        $$
        ...
        *   **分支点数**: 骨架上一个点连接了三条或更多路径的点，其数量反映了形状的复杂分支结构。一个像素 `p` 是分支点，当且仅当 $C(p) > 2$。
            $$
            N_{branch} = .um_{p .n S} .athbb{I}(C(p) > 2) .uad .ext{其中 } .athbb{I}(.dot) .ext{ 是指示函数}
            $$
        *   **端点数**: 骨架上只连接了一条路径的点，反映了形状末梢的数量。一个像素 `p` 是端点，当且仅当 $C(p) = 1$。
            $$
            N_{end} = .um_{p .n S} .athbb{I}(C(p) = 1)
            $$
...
*   **B样条插值算法**:
    ...
    *   **基本情况 (0次)**:
        $$
        N_{i,0}(t) = .egin{cases} 1 & .ext{if } u_i .e t < u_{i+1} . 0 & .ext{otherwise} .nd{cases}
        $$
    *   **递归步骤 ($p>0$)**:
        $$
        N_{i,p}(t) = .rac{t - u_i}{u_{i+p} - u_i} N_{i,p-1}(t) + .rac{u_{i+p+1} - t}{u_{i+p+1} - u_{i+1}} N_{i+1,p-1}(t)
        $$
    ...
    $$
    Q_j = C(t_j) = .um_{i=0}^{n} P_i N_{i,p}(t_j) .uad .ext{for } j=0, .ots, n
    $$
...
*   **关键特征**:
    *   **能量**: $.ext{Energy} = .um_{i=1}^{N_g} p(i)^2$ (衡量图像强度分布的均匀性)
    *   **熵**: $.ext{Entropy} = -.um_{i=1}^{N_g} p(i) .og_2(p(i))$ (衡量图像强度的随机性或复杂性)
    *   **均值**, **标准差**, **偏度**, **峰度** 等标准统计量。
...
2.  **特征提取原理**:
    ...
    3.  **获取Patch令牌**: 预处理后的2D图像张量输入到Radio-DINO模型。我们提取模型最后一个Transformer Block的输出，该输出是一个包含了所有图像块的高维表示的序列，称为Patch令牌。
        $$
        .ext{Tokens}_{.ext{patch}} = .ext{ViT}_{.ext{final.layer}}(.ext{Image}_{.ext{preprocessed}})
        $$
...
- 标量：小写字母（例如 $x$）。
- 向量：粗体小写字母（例如 $.athbf{x}$），$.athbf{x}.n.athbb{R}^I$。
- 矩阵：粗体大写字母（例如 $.athbf{X}$），$.athbf{X}.n.athbb{R}^{I.imes J}$。
- 张量：花体大写字母（例如 $.athcal{X}$），$N$ 阶张量为
  $$
  .athcal{X}.n.athbb{R}^{I_1.imes I_2.imes.dots.imes I_N}.
  $$
  向量是 1 阶张量，矩阵是 2 阶张量。

**模‑$n$ 展开**

张量的模-$n$ 展开是将高维张量重新排列为矩阵的过程。对于 $N$ 阶张量 $.athcal{X} .n .athbb{R}^{I_1 .imes I_2 .imes .dots .imes I_N}$，其模-$n$ 展开记为 $.athbf{X}_{(n)}$。

定义：$.athbf{X}_{(n)} .n .athbb{R}^{I_n .imes (I_1 .dots I_{n-1} I_{n+1} .dots I_N)}$ 的列向量由张量的模-$n$ fibers 组成。具体地，张量元素 $(i_1, i_2, .ots, i_N)$ 映射到矩阵元素 $(i_n, j)$，其中
$$
j = 1 + .um_{k=1, k .eq n}^{N} (i_k - 1) J_k, .uad .ext{with } J_k = .rod_{m=1, m .eq n}^{k-1} I_m.
$$
直观上，$.athbf{X}_{(1)}$ 是将张量的“列”作为列向量排列，$.athbf{X}_{(2)}$ 是将张量的“行”作为列向量排列，$.athbf{X}_{(3)}$ 是将张量的“管” 作为列向量排列。

**模‑$n$ 张量–矩阵乘积**

$N$ 阶张量 $.athcal{X} .n .athbb{R}^{I_1 .imes I_2 .imes .dots .imes I_N}$ 与矩阵 $.athbf{U} .n .athbb{R}^{J .imes I_n}$ 的模-$n$ 乘积记为 $.athcal{X} .imes_n .athbf{U}$，结果为一个 $N$ 阶张量，其尺寸为 $I_1 .imes .dots .imes I_{n-1} .imes J .imes I_{n+1} .imes .dots .imes I_N$。

定义
$$
(.athcal{X} .imes_n .athbf{U})_{i_1 .ots i_{n-1} j i_{n+1} .ots i_N} = .um_{i_n=1}^{I_n} x_{i_1 .ots i_n .ots i_N} u_{j i_n}
$$

这等价于
$$
.athcal{Y} = .athcal{X} .imes_n .athbf{U} .ff .athbf{Y}_{(n)} = .athbf{U} .athbf{X}_{(n)}
$$

**Frobenius 范数**

对张量 $.athcal{X}$，Frobenius 范数定义为
$$
..athcal{X}._F
=.qrt{.um_{i_1,.ots,i_N} x_{i_1.dots i_N}^2},
$$
等价于把所有元素当作一个长向量后的 $.ll^2$ 范数。

**核范数**

对矩阵 $M$，其奇异值为 $..igma_k.$ 时，核范数为
$$
.M._* = .um_k .igma_k,
$$
...
Tucker 分解可被视为高阶的主成分分析（PCA）。对于三阶张量 $.athcal{X} .n .athbb{R}^{I_1 .imes I_2 .imes I_3}$，其分解形式为：
$$
.athcal{X} .pprox .athcal{G} .imes_1 .athbf{U}^{(1)} .imes_2 .athbf{U}^{(2)} .imes_3 .athbf{U}^{(3)}
$$
其中：
- $.athcal{G} .n .athbb{R}^{R_1 .imes R_2 .imes R_3}$ 为 Core Tensor，描述了不同模态因子之间的相互作用水平，其元素值反映了对应特征组合的能量或重要性。
- $.athbf{U}^{(n)} .n .athbb{R}^{I_n .imes R_n}$ 为正交的因子矩阵，通常包含该模态下的主成分基向量。
...
$$
.athcal{X}.n.athbb{R}^{I_1.imes I_2.imes I_3},
$$
CP 分解写作
$$
.at{.athcal{X}}
=.um_{r=1}^R .ambda_r. a_r.irc b_r.irc c_r,
$$
其中
- $.ambda_r.n.athbb{R},.a_r.n.athbb{R}^{I_1},.b_r.n.athbb{R}^{I_2},.c_r.n.athbb{R}^{I_3}$；
- $.irc$ 为外积运算；
- $R$ 为 CP rank。
...
C=[c_1,.ots,c_R],
$$
则 $.at{.athcal{X}}$ 完全由 $(A,B,C,.ambda_1,.ots,.ambda_R)$ 决定。

**软阈值算子**

对于标量 $x .n .athbb{R}$ 和阈值 $.au > 0$，软阈值算子定义为：
$$
S_.au(x) = .peratorname{sgn}(x) .ax(|x| - .au, 0)
$$
...
对单个椎间盘 ROI $.athcal{X}.n.athbb{R}^{I_x.imes I_y.imes I_z}$ 做 HOSVD / Tucker 分解。该过程旨在将高维 MRI 信号投影到由数据驱动的正交基底上，捕获其在三个空间方向上的“主变形模式”和整体结构复杂度：
...
对 $.athcal{X}.n.athbb{R}^{I_x.imes I_y.imes I_z}$，$.athcal{X}$ 可以被视为“信号（结构信息）”与“噪声”的叠加。HOSVD 通过保留核心张量中能量较大的部分，重构出一个低秩近似张量 $.at{.athcal{X}}$。$.at{.athcal{X}}$ 是 $.athcal{X}$ 在低维子空间上的投影。残差 $.athcal{E} = .athcal{X} - .at{.athcal{X}}$ 包含了高频噪声或非典型的微细纹理。我们将 $.at{.athcal{X}}$ 的核心能量特征（代表主要解剖结构）与残差 $.athcal{E}$ 的能量特征（代表纹理复杂度和噪声水平）同时作为诊断特征。HOSVD 算法步骤如下：

1. 分别按 3 个模展开并做 SVD：
   $$
   X_{(1)} = U^{(1)}.igma^{(1)}V^{(1).op},.uad
   X_{(2)} = U^{(2)}.igma^{(2)}V^{(2).op},.uad
   X_{(3)} = U^{(3)}.igma^{(3)}V^{(3).op},
   $$
   其中 $U^{(n)}$ 为左奇异向量矩阵，$.igma^{(n)}$ 为奇异值对角矩阵。
2. 得到 HOSVD 形式：
   $$
   .athcal{X}
   =.athcal{S}.imes_1 U^{(1)}.imes_2 U^{(2)}.imes_3 U^{(3)}.
   $$
3. 为获得低秩近似，需选定多线性截断秩 $(R_1,R_2,R_3)$。对每个模 $n=1,2,3$，令奇异值为
   $..igma^{(n)}_k._{k.e 1}$，选最小 $R_n$ 使得
   $$
   .rac{.um_{k=1}^{R_n}.igl(.igma^{(n)}_k.igr)^2}
        {.um_k .igl(.igma^{(n)}_k.igr)^2}
   .e .ta,.quad .ta.pprox 0.95.
   $$
4. 构造截断因子矩阵
   $$
   .ilde{U}^{(n)} = U^{(n)}(:,1:R_n),
...
1. 各模主奇异值向量
   ...
   取每个模的前 $K_n$ 个奇异值，$K_n.pprox 10.im 15$：
   $$
   f^{(n)}_{.igma}
   =.igl[.igma^{(n)}_1,.ots,.igma^{(n)}_{K_n}.igr].
   $$
   这些值反映该模整体能量及其衰减速度。
...
2. 各模能量分布
   ...
   $$
   e^{(n)}_k
   =.rac{.igl(.igma^{(n)}_k.igr)^2}
          {.um_j .igl(.igma^{(n)}_j.igr)^2},
   .uad k=1,.ots,K_n,
   $$
   刻画能量是否集中在少数主方向上。
...
3. core tensor 能量与方向“重要性”分布
   ...
   - 核心总能量：
     $$
     E_{.ext{core}}=..ilde{.athcal{S}}._F^2.
     $$
   - 沿第 1 模的能量分布：
     $$
     p_1(i)
     =.rac{.um_{j,k}.ilde{.athcal{S}}_{ijk}^2}
            {..ilde{.athcal{S}}._F^2},.uad i=1,.ots,R_1,
     $$
     类似可定义 $p_2(j),p_3(k)$。
...
由于 MRI 的 Rician 噪声具有信号依赖性，首先对原始椎间盘 ROI 数据 $.athcal{Y}_{raw}$ 进行前向 VST 变换，将其转化为近似的高斯白噪声分布数据 $.athcal{Y}$：
$$
.athcal{Y} = f_{VST}(.athcal{Y}_{raw}, .igma_n) = .qrt{.athcal{Y}_{raw}^2 + .igma_{n}^2}
$$
其中 $.igma_n$ 为预估的背景噪声标准差。

2. Method Noise 外部迭代

为了找回在去噪过程中可能丢失的Method Noise，算法引入外部反馈机制。设定外部迭代次数 $Iter$（通常取 2），在第 $k$ 次迭代中：
$$
.athcal{Y}^{(k)} = .athcal{X}^{(k-1)} + .lpha (.athcal{Y} - .athcal{X}^{(k-1)})
$$
其中 $.athcal{Y}$ 是初始 VST 数据，$.athcal{X}^{(k-1)}$ 是上一轮的恢复结果（初始 $.athcal{X}^{(0)} = .athcal{Y}$），$.lpha$ 为反馈系数。

3. Patch 组张量构造
   
对当前的输入 $.athcal{Y}^{(k)}$，利用非局部自相似性构建张量：
- 分块：以步长 $m-1$ 滑动，提取尺寸为 $m .imes m .imes m$ 的参考块。
- 搜索与匹配：在 $s .imes s .imes s$ 的搜索窗口内，寻找与参考块欧氏距离最小的 $n$ 个相似块。
- 堆叠：将这 $n$ 个相似块沿第 4 维堆叠，形成观测张量 $.athcal{G}_{.athcal{Y}} .n .athbb{R}^{m .imes m .imes m .imes n}$。

##### 1.4.4.3 对数和正则化模型 (NLRTA-LSR)

对每个 patch 组张量 $.athcal{G}_{.athcal{Y}}$，旨在寻找低秩近似张量 $.athcal{G}_{.athcal{X}}$，优化目标函数定义为：
$$
.in_{.athcal{G}_{.athcal{X}}} .um_{i=1}^{4} .heta_i .igl..athcal{G}_{.athcal{X},(i)}.igr._{log} .uad .ext{s.t.} .uad ..athcal{G}_{.athcal{Y}} - .athcal{G}_{.athcal{X}}._F^2 .e .arepsilon
$$
转化为无约束拉格朗日形式：
$$
.in_{.athcal{G}_{.athcal{X}}} .um_{i=1}^{4} .heta_i .igl..athcal{G}_{.athcal{X},(i)}.igr._{log} + .rac{.u}{2} ..athcal{G}_{.athcal{Y}} - .athcal{G}_{.athcal{X}}._F^2
$$

其中：
- $.athcal{G}_{.athcal{X},(i)}$ 是张量沿第 $i$ 模的展开矩阵；
- $.heta_i$ 为权重，满足 $.um .heta_i = 1$且 $.heta_i .e 0$；
- $..athbf{A}._{log}$ 为矩阵的**对数和范数**，定义为：
  $$
  ..athbf{A}._{log} = .um_{j=1}^{r} .og(.elta_j(.athbf{A}) + .arepsilon)
  $$
  这里 $.elta_j(.athbf{A})$ 是矩阵 $.athbf{A}$ 的第 $j$ 个奇异值，$.arepsilon$ 是一个微小的常数用于保持数值稳定性。
该模型是非凸的，能比核范数更强地诱导低秩性，同时对大奇异值（重要特征）的惩罚更小。

##### 1.4.4.4 数值求解：ADMM 与加权奇异值阈值

采用交替方向乘子法（ADMM）将上述问题分解为子问题求解。引入辅助变量 $.athcal{O}_i = .athcal{X}_{group}$，将原问题转化为约束优化，并构造增广拉格朗日函数。ADMM 迭代核心在于求解辅助变量 $.athcal{O}_i$ 的更新。

1. 更新辅助变量 $.athcal{O}_i$ (转化为 WNNM 问题)：
   固定其他变量，求解 $.athcal{O}_i$ 等价于求解加权核范数最小化（WNNM）问题：
   $$
   .athcal{O}_i^{t+1} = .rg.in_{.athcal{O}_i} .rac{.heta_i}{.u^t} ..athcal{O}_{i,(i)}._{log} + .rac{1}{2} ..athcal{O}_i - .athcal{T}_i^t._F^2
   $$
   其中:
   - $.athcal{T}_i^t = .athcal{G}_{.athcal{X}}^t - .rac{.athcal{Q}_i^t}{.u^t}$ ($.athcal{Q}_i$ 为拉格朗日乘子)。
   - $.athcal{O}_i^{t+1}$：本轮迭代中，基于第 $i$ 模态展开矩阵奇异值收缩后折叠回来的张量。
   - $.athcal{Q}_i^t$：第 $i$ 个约束对应的拉格朗日乘子张量（记录了上一轮的残差信息）。
   - $.u^t$：当前的惩罚参数。
   
   根据 Wang, Z., et al. (2022) Lemma 1，该问题有闭式解，通过加权奇异值阈值算子(WSVT)求解。设 $.athcal{T}_{i,(i)}$ 的奇异值分解为 $.athbf{U}.athbf{.igma}.athbf{V}^T$，则更新后的奇异值 $.i_j$ 为：
   $$
   .i_j(.athcal{O}_{i,(i)}) = .ax.eft(0, .rac{c_1 + .qrt{c_2}}{2}.ight) .uad .ext{if } c_2 .e 0, .ext{ else } 0
   $$
   其中：
   - $c_1 = .elta_j(.athcal{T}_{i,(i)}) - .arepsilon$
   - $c_2 = (.elta_j(.athcal{T}_{i,(i)}) + .arepsilon)^2 - .rac{4.heta_i}{.u^t}$
   
   **注意**：此处产生的自适应权重隐含在阈值操作中，其等效权重 $.mega_j$ 与奇异值大小成反比，即 $.mega_j = .rac{1}{.elta_j + .arepsilon}$。

2. 更新全局张量 $.athcal{G}_{.athcal{X}}$：
   $$
   .athcal{G}_{.athcal{X}}^{t+1} = .rac{1}{4} .um_{i=1}^{4} .eft( .athcal{O}_i^{t+1} + .rac{.athcal{Q}_i^t}{.u^t} .ight)
   $$

3. 更新乘子 $.athcal{Q}_i$ 与惩罚参数 $.u$：
   $$
   .athcal{Q}_i^{t+1} = .athcal{Q}_i^t + .u^t (.athcal{O}_i^{t+1} - .athcal{G}_{.athcal{X}}^{t+1})
   $$
   $$
   .u^{t+1} = .ho .u^t
   $$
   其中 $.ho$ 是一个大于 1 的常数（取 $.ho .pprox 1.05 .im 1.1$），保证算法在后期强制收敛。

4. 收敛性检查: 
   计算相对误差，若小于阈值（如 $.psilon = 10^{-4}$）则停止迭代：
   $$
   .rac{..athcal{G}_{.athcal{X}}^{t+1} - .athcal{G}_{.athcal{X}}^{t}._F^2}{..athcal{G}_{.athcal{X}}^{t}._F^2} .e .psilon
   $$

##### 1.4.4.5 特征构造

利用上述算法对每个 patch 组优化后的低秩张量 $.at{.athcal{X}}_{group}$ 及其过程参数提取特征：

1. 各模态的奇异值

   对最终恢复的低秩张量 $.at{.athcal{X}}_{group}$ 进行四个方向的模态展开，得到矩阵 $.at{.athcal{X}}_{(n)}$ ($n=1,2,3,4$)。对每个矩阵进行 SVD 分解，获取奇异值向量：
   $$
   .oldsymbol{.elta}^{(n)} = [.elta_1^{(n)}, .elta_2^{(n)}, .ots, .elta_{r}^{(n)}]
   $$

2. 自适应权重向量

   利用公式计算对应的权重向量 $.oldsymbol{.mega}^{(n)}$：
   $$
   .mega_j^{(n)} = .rac{1}{.elta_j^{(n)} + .arepsilon}
   $$
   同时计算均值和标准差。其中 $.arepsilon$ 是一个极小正常数（如 $10^{-16}$）。

3. 对数和正则化强度

   计算每个模态的对数和范数值，这反映了在该模态下数据的稀疏程度：
   $$
   L^{(i)} = .um_{j} .og(.elta_j^{(i)} + .arepsilon)
   $$
   同时计算$L^{(1)}, .ots, L^{(4)}$ 的均值。


4. 非局部自相似残差
   
   $$
   e_{NL} = .rac{..athcal{G}_{.athcal{Y}} - .at{.athcal{G}}_{.athcal{X}}._F^2}{..athcal{G}_{.athcal{Y}}._F^2}
   $$
   同时计算所有 patch 组 $e_{NL}$ 的均值和标准差，反映该区域是否符合“非局部低秩”假设，病变区域往往具有异常的结构复杂性。

5. 对数和有效秩

   对最终低秩张量 $.at{.athcal{G}}_{.athcal{X}}$ 的 4 个模态展开矩阵，分别计算：
   $$
   R_{log}^{(i)} = .um_{j} .og.eft(.elta_j(.at{.athcal{G}}_{.athcal{X},(i)}) + .arepsilon.ight), .uad i=1,2,3,4
   $$

   计算 ROI 内所有 Patch 组的 $R_{log}^{(i)}$ 的均值，构成 4 维特征向量：
   $$
   .athbf{f}_{rank} = .eft[ .ext{mean}(R_{log}^{(1)}), .ext{mean}(R_{log}^{(2)}), .ext{mean}(R_{log}^{(3)}), .ext{mean}(R_{log}^{(4)}) .ight]
   $$
...
$$
.athcal{X}.n.athbb{R}^{I_x.imes I_y.imes I_z},
$$
考虑秩为 $R$ 的 CP 分解：
$$
.at{.athcal{X}}
=.um_{r=1}^{R}.ambda_r.dot.athbf{a}_r.irc.athbf{b}_r.irc.athbf{c}_r,
$$
其中：

- $.ambda_r.n.athbb{R}$ 为第 $r$ 个秩‑1 成分的权重；
- $.athbf{a}_r.n.athbb{R}^{I_x}$、$.athbf{b}_r.n.athbb{R}^{I_y}$、$.athbf{c}_r.n.athbb{R}^{I_z}$ 分别为三条空间方向上的因子向量；
- $.irc$ 为向量外积。

将因子列向量堆叠成矩阵：
$$
.athbf{A}=[.athbf{a}_1,.ots,.athbf{a}_R].n.athbb{R}^{I_x.imes R},.uad
.athbf{B}=[.athbf{b}_1,.ots,.athbf{b}_R].n.athbb{R}^{I_y.imes R},.uad
.athbf{C}=[.athbf{c}_1,.ots,.athbf{c}_R].n.athbb{R}^{I_z.imes R}.
$$

约定在分解的任何时刻都保持
$$
..athbf{a}_r._2=..athbf{b}_r._2=..athbf{c}_r._2=1,
$$
所有尺度信息集中到 $.ambda_r$ 中。此时若忽略成分之间的非正交性，可以把 $.ambda_r^2$ 近似理解为第 $r$ 个模式所携带的“能量大小”。
...
2. 强度裁剪与标准化
    ...
    - 在 ROI 内做 $z$‑score 标准化：
      $$
      x'_{ijk}
      =.rac{x_{ijk}-.u_{.athcal{X}}}{.igma_{.athcal{X}}},
      $$
      其中 $.u_{.athcal{X}},.igma_{.athcal{X}}$ 均为本 ROI 体素的均值和标准差。

后续 CP 分解都在标准化后的张量上进行。为简化记号，下文仍记为 $.athcal{X}$。

##### 1.4.5.4 CP 分解的 ALS 求解

对单个三阶张量 $.athcal{X}$，采用交替最小二乘（ALS）求解 CP 分解，目标函数为最小化重构误差：
$$
.in_{.athbf{A},.athbf{B},.athbf{C},.oldsymbol{.ambda}}
.eft.
.athcal{X}
-.um_{r=1}^R .ambda_r.dot .athbf{a}_r.irc .athbf{b}_r.irc .athbf{c}_r
.ight._F^2.
...
1. 初始化（直接用 HOSVD 结果初始化）
    - 对 $.athcal{X}$ 做三模展开：
      ...
      并各自做 SVD：
      $$
      X_{(1)}=U^{(1)}.igma^{(1)}V^{(1).op},\
      X_{(2)}=U^{(2)}.igma^{(2)}V^{(2).op},\
      X_{(3)}=U^{(3)}.igma^{(3)}V^{(3).op}.
      $$
    - 取前 $R$ 列作为初始因子矩阵：
      $$
    ...
    - 初始权重：
      $$
      .ambda_r^{(0)}
      =.angle.athcal{X},.athbf{a}_r^{(0)}.irc.athbf{b}_r^{(0)}.irc.athbf{c}_r^{(0)}.angle
      =.um_{i,j,k}x_{ijk}.athbf{a}^{(0)}_{r,i}.athbf{b}^{(0)}_{r,j}.athbf{c}^{(0)}_{r,k}.
      $$
2. ALS 迭代更新因子矩阵
   ...
   $$
   .athbf{C}.st.athbf{B}
   =[.athbf{c}_1.times.athbf{b}_1,.ots,.athbf{c}_R.times.athbf{b}_R]
   .n.athbb{R}^{(I_yI_z).imes R},
   $$
   记 $.dot$ 为 Hadamard 乘积。
   ...
   在第 $t$ 轮迭代中：
    - 更新 $.athbf{A}$：
      $$
      .athbf{A}^{(t+1)}
      = X_{(1)}.igl(.athbf{C}^{(t)}.st.athbf{B}^{(t)}.igr)
      .eft(
      .igl(.athbf{C}^{(t).op}.athbf{C}^{(t)}.igr).dot
      .igl(.athbf{B}^{(t).op}.athbf{B}^{(t)}.igr)
      .ight)^{.agger},
      $$
    - 更新 $.athbf{B}$：
      $$
      .athbf{B}^{(t+1)}
      = X_{(2)}.igl(.athbf{C}^{(t)}.st.athbf{A}^{(t+1)}.igr)
      .eft(
      .igl(.athbf{C}^{(t).op}.athbf{C}^{(t)}.igr).dot
      .igl(.athbf{A}^{(t+1).op}.athbf{A}^{(t+1)}.igr)
      .ight)^{.agger},
      $$
    - 更新 $.athbf{C}$：
      $$
      .athbf{C}^{(t+1)}
      = X_{(3)}.igl(.athbf{B}^{(t+1)}.st.athbf{A}^{(t+1)}.igr)
      .eft(
      .igl(.athbf{B}^{(t+1).op}.athbf{B}^{(t+1)}.igr).dot
      .igl(.athbf{A}^{(t+1).op}.athbf{A}^{(t+1)}.igr)
      .ight)^{.agger}.
      $$
3. 列归一化与权重更新
   ...
   为保持稳定性和可解释性，在每轮迭代末，对每个成分 $r$：
    - 计算各模向量的范数：
      $$
      s_a=..athbf{a}_r^{(t+1)}._2,.uad
      s_b=..athbf{b}_r^{(t+1)}._2,.uad
      s_c=..athbf{c}_r^{(t+1)}._2;
      $$
    - 归一化列向量：
      $$
      .athbf{a}_r^{(t+1)}.eftarrow.rac{.athbf{a}_r^{(t+1)}}{s_a},.uad
      .athbf{b}_r^{(t+1)}.eftarrow.rac{.athbf{b}_r^{(t+1)}}{s_b},.uad
      .athbf{c}_r^{(t+1)}.eftarrow.rac{.athbf{c}_r^{(t+1)}}{s_c};
      $$
    - 将缩放系数累积到权重：
      $$
      .ambda_r^{(t+1)}.eftarrow .ambda_r^{(t)}.dot s_a s_b s_c.
      $$
4. 收敛条件

   $$
   .at{.athcal{X}}^{(t)}
   =.um_{r=1}^R.ambda_r^{(t)}.athbf{a}_r^{(t)}.irc.athbf{b}_r^{(t)}.irc.athbf{c}_r^{(t)},.uad
   .athcal{E}^{(t)}=.athcal{X}-.at{.athcal{X}}^{(t)}.
   $$

   若
   $$
   .rac{..athcal{E}^{(t)}._F^2-..athcal{E}^{(t-1)}._F^2}
   {..athcal{X}._F^2}
   .e .arepsilon_{.ext{ALS}}
   $$
   如果 $.arepsilon_{.ext{ALS}}=10^{-4}$，或迭代次数超过 $T{.ax}$（如 1000），则停止。

##### 1.4.5.5 秩 $R$ 的选取

- 经验上，CP 秩通常取一个较小到中等的整数以避免病态问题，医学图像应用中多取 $R.n[3,10]$；
- 默认：$R=10$

可通过以下方式检验：对每个 ROI 计算重构误差比例
$$
r_{.ext{CP}}
=.rac{..athcal{X}-.at{.athcal{X}}._F^2}{..athcal{X}._F^2},
$$
若在绝大多数 ROI 上 $r_{.ext{CP}}<0.05.im 0.1$，说明 $R$ 足以表达主要结构。

##### 1.4.5.6 特征构造

在得到单 ROI 的 CP 分解 $(.oldsymbol{.ambda},.athbf{A},.athbf{B},.athbf{C})$ 后，从三个层面构造特征：
...
1. 权重与能量谱特征
   ...
   - 找到排列 $.i$ 使得
     $$
     |.ambda_{.i(1)}|.e|.ambda_{.i(2)}|.e.dots.e|.ambda_{.i(R)}|;
     $$
   - 定义重排序后的权重和因子：
     $$
     .ilde{.ambda}_r=.ambda_{.i(r)},.uad
     .ilde{.athbf{a}}_r=.athbf{a}_{.i(r)},\
     .ilde{.athbf{b}}_r=.athbf{b}_{.i(r)},\
     .ilde{.athbf{c}}_r=.athbf{c}_{.i(r)}.
     $$
   ...
   - 主权重向量：
      $$
      f_{.ambda}
      =[.ilde{.ambda}_1,.ots,.ilde{.ambda}_R]^T.
      $$
   - 相对能量比例：
      $$
      E_{.ambda}
      =.um_{r=1}^R .ilde{.ambda}_r^2,.quad
      e_r
      =.rac{.ilde{.ambda}r^2}{E_{.ambda}+.arepsilon},.r=1,.ots,R,
      $$
      其中 $.arepsilon$ 防止除零。向量 $f_e=[e_1,.ots,e_R]^T$ 描述能量是否集中在少数几个成分。
   - 有效 CP 秩：
      $$
      R_{.ext{eff}}
      =.rac{.eft(.um_{r=1}^R .ilde{.ambda}_r^2.ight)^2}
      {.um_{r=1}^R .ilde{.ambda}_r^4+.arepsilon}.
      $$

2. 因子向量的“集中度”与熵

   对每个成分 $r$ 和每个模（以 $.ilde{.athbf{a}}_r$ 为例），把平方归一化为一维分布：
   $$
   p^{(a)}_{r,i}
   =.rac{.igl(.ilde{.athbf{a}}_{r,i}.igr)^2}{.um_{k=1}^{I_x}.igl(.ilde{.athbf{a}}_{r,k}.igr)^2},
   .uad i=1,.ots,I_x,
   $$
   ...
   - 熵：
      $$
      H^{(a)}_r
      =-.um_{i=1}^{I_x}p^{(a)}_{r,i}.og(p^{(a)}_{r,i}+.arepsilon),
      $$
      $$
   ...
   - 集中度（类似 Gini 指标）：
      $$
      G^{(a)}_r
      =.um_{i=1}^{I_x}.igl(p^{(a)}_{r,i}.igr)^2,
      $$
      $$
   ...
   为控制维度，只对前 $K$ 个最重要成分统计这些量，取 $K=.in(3,R)$；特征集合为：
     $$
...
3. 整体强度
   ...
   $$
   S_{.ambda}
   =.um_{r=1}^R|.ilde{.ambda}_r|,.quad
   ..oldsymbol{.ilde{.ambda}}._2
   =.eft(.um_{r=1}^R.ilde{.ambda}_r^2.ight)^{1/2}.
   $$
...
| `energy_threshold` | `0.95` | 能量阈值 $.ta$。对每个模的奇异值序列，选择最小的多线性秩 $R_n$，使前 $R_n$ 个奇异值的能量比例 $.um_{k=1}^{R_n}.igma_k^2 / .um_j .igma_j^2 .e .ta$。该值越接近 1，保留的主模态越多，重构误差越小，但特征维度增加。 |
...
| `similar_patches` | `64` | 相似块数量 $n$。对每个参考 patch，在非局部搜索窗口中取最相似的 $n$ 个块堆叠成四阶张量 $.athcal{G}_{.athcal{Y}}$。 |
| `search_window` | `15` | 搜索窗口边长 $s$（体素数）。决定在多大空间范围内寻找自相似 patch。 |
| `internal_iterations` | `50` | ADMM 内部迭代次数 $T$。|
| `epsilon` | `1e-16` | 对数和范数中的平滑常数 $.arepsilon$，防止 $.og(0)$ 并稳定权重计算。 |
| `alpha_feedback` | `0.1` | Method Noise 外部迭代的反馈系数 $.lpha$，控制噪声残差注入的强度。 |
| `beta_noise` | `0.3` | 噪声估计参数 $.eta$，用于在 ROI 中估计 Rician 噪声标准差 $.igma_n$。 |
...
| `max_iter` | `1000` | CP‑ALS 最大迭代次数 $T_{.ax}$。若在 `max_iter` 内达到 `tol` 指定的收敛标准则提前停止。 |
| `tol` | `1e-4` | CP‑ALS 收敛阈值 $.arepsilon_{.ext{ALS}}$。两次迭代的目标函数变化比例小于该值时认为收敛。 |
| `epsilon_cp` | `1e-6` | CP 特征构造中的平滑常数 $.arepsilon$，用于计算权重能量比例、有效秩以及因子向量熵/集中度时防止除零或 $.og(0)$ 引起的数值不稳定。通常保持默认即可。 |
| `top_components` | `3` | 参与计算因子向量熵 $H$ 和集中度 $G$ 的前 $K$ 个主成分个数。实际使用时取 $.in(K, R)$，方案默认 `K=3`。 |
...
*   **原理**:
    膨胀是一种基本的形态学操作。对于一个二值化的掩码图像 `A` 和一个被称为结构元素的小模板 `B`，`A` 被 `B` 膨胀的结果，记为 $A .plus B$，是所有点 `z` 的集合，使得 `B` 平移 `z` 后与 `A` 的交集不为空。
    $$
    A .plus B = .z | (.at{B})_z .ap A .eq .mptyset .
    $$
    其中 $.at{B}$ 是 `B` 的映像。在实际计算中，该操作通过以下算法实现：
...
*   **原理**:
    腐蚀是膨胀的对偶操作。`A` 被 `B` 腐蚀的结果，记为 $A .minus B$，是所有点 `z` 的集合，使得 `B` 平移 `z` 后完全包含于 `A` 中。
    $$
    A .minus B = .z | (B)_z .ubseteq A .
    $$
...
*   **原理**:
    ...
    3.  **执行随机形态学操作**: 使用上一步随机生成的参数，对掩码执行选定的膨胀或腐蚀操作。
        $$
        .ext{Mask}_{.ext{perturbed}} = .egin{cases} .ext{Mask}_{.ext{original}} .plus B_{.ext{rand}} & .ext{if } p > 0.5 . .ext{Mask}_{.ext{original}} .minus B_{.ext{rand}} & .ext{if } p .e 0.5 .nd{cases}
        $$
...
*   **原理**:
    ...
    2.  **构建变换矩阵**:
        ...
        $$
        M = .egin{bmatrix} 1 & 0 & t_x . 0 & 1 & t_y .nd{bmatrix}
        $$
    3.  **应用变换**:
        将此矩阵 `M` 应用于原始图像和掩码，计算出每个像素的新位置：
        $$
        .egin{bmatrix} x' . y' .nd{bmatrix} = .egin{bmatrix} 1 & 0 & t_x . 0 & 1 & t_y .nd{bmatrix} .egin{bmatrix} x . y . 1 .nd{bmatrix} = .egin{bmatrix} x + t_x . y + t_y .nd{bmatrix}
        $$
...
*   **原理与算法**:
    ...
    2.  **定义高斯分布**:
        该高斯分布的概率密度函数由以下公式定义：
        $$
        P(z) = .rac{1}{.igma .qrt{2.i}} e^{ - .rac{(z - .u)^2}{2.igma^2} }
        $$
        根据代码实现，该分布的参数被设定为：
        *   **均值 $.u$**: 固定为 0。
        *   **标准差 $.igma$**: 由用户在界面中设置的噪声标准差（`noise_std`）参数决定。
    3.  **应用噪声**:
...
| **噪声标准差** | 高斯噪声、组合扰动 | 定义了所添加高斯噪声分布的标准差 $.igma$（均值固定为0）。值越大，添加到图像中的噪声强度越高，图像信噪比越低。 |
...
#### I. 组内相关系数
  ...
  * **计算原理**:
    ...
    2.  **计算基本统计量**:
        *   总均值 $.ar{x}$: 所有 $x_{ij}$ 的平均值。
        *   行均值 $.ar{x}_{i.}$: 第 $i$ 个病例在所有条件下的平均值。
        *   列均值 $.ar{x}_{.j}$: 第 $j$ 个条件在所有病例下的平均值。

    3.  **计算离差平方和**:
        *   **总离差平方和**: 反映数据的总变异。
            $$
            SS_{.ext{总}} = .um_{i=1}^{n} .um_{j=1}^{k} (x_{ij} - .ar{x})^2
            $$
        *   **行间离差平方和 (病例间)**: 反映由不同病例引起的变异。
            $$
            SS_{.ext{行}} = k .um_{i=1}^{n} (.ar{x}_{i.} - .ar{x})^2
            $$
        *   **列间离差平方和 (条件间)**: 反映由不同扰动条件引起的变异。
            $$
            SS_{.ext{列}} = n .um_{j=1}^{k} (.ar{x}_{.j} - .ar{x})^2
            $$
    ...
    4.  **计算均方**:
        ...
        *   病例间均方: $MS_{.ext{行}} = .rac{SS_{.ext{行}}}{n-1}$
        *   误差均方: $MS_{.ext{误差}} = .rac{SS_{.ext{误差}}}{(n-1)(k-1)}$

    5.  **计算组内相关系数值**:
        最终，`ICC(3,k)`的值由病例间均方和误差均方计算得出，它量化了病例间的真实变异占总变异（真实变异+误差变异）的比例。
        $$
        .ext{ICC}(3,k) = .rac{MS_{.ext{行}} - MS_{.ext{误差}}}{MS_{.ext{行}}}
        $$
...
#### II. 分层聚类
  ...
  * **原理与算法**:
    ...
    1.  **特征向量化**: 将每个特征的稳健性表现抽象为一个向量 $.athbf{f}$，向量的维度等于扰动条件的数量，每个分量是该特征在对应条件下的组内相关系数值。
        $$
        .athbf{f}_{.ext{特征A}} = [.ext{ICC}_{.ext{条件1}}, .ext{ICC}_{.ext{条件2}}, .ots, .ext{ICC}_{.ext{条件k}}]
        $$

    2.  **距离矩阵计算**: 使用欧几里得距离计算每两个特征向量之间的距离，形成一个距离矩阵。向量 $.athbf{a}$ 和 $.athbf{b}$ 之间的距离定义为：
        $$
        d(.athbf{a}, .athbf{b}) = .qrt{.um_{i=1}^{k} (a_i - b_i)^2}
        $$
    ...
    3.  **Ward链接算法**:
        ...
        *   **迭代合并**: 在每一步中，算法会计算所有可能的簇对合并后，所导致的总簇内误差平方和的增量。
            *   单个簇 $C_m$ 的误差平方和定义为簇内所有特征向量到该簇质心（均值向量）$.athbf{.u}_m$ 的平方距离之和：
                $$
                E_m = .um_{.athbf{f}_i .n C_m} ||.athbf{f}_i - .athbf{.u}_m||^2
                $$
            *   总误差平方和是所有簇的误差平方和之和: $E_{.ext{总}} = .um_{m} E_m$。
...
#### III. Spearman相关性分析
  ...
  * **原理与算法**:
    ...
    1.  **计算相关系数**:
        ...
        $$
        .ho = .rac{.um_{i=1}^{n}(rg_{X_i} - .verline{rg_X})(rg_{Y_i} - .verline{rg_Y})}{.qrt{.um_{i=1}^{n}(rg_{X_i} - .verline{rg_X})^2 .um_{i=1}^{n}(rg_{Y_i} - .verline{rg_Y})^2}}
        $$
        其中，$rg_{X_i}$ 和 $rg_{Y_i}$ 是第 $i$ 个病例在特征X和Y上的秩次，$.verline{rg_X}$ 和 $.verline{rg_Y}$ 分别是两个秩次变量的平均值，$n$ 是病例总数。

        这个值 $.ho$ 就是Spearman相关系数，它衡量了两个原始变量之间单调关系的强度，而对具体数值分布和异常值不敏感。
