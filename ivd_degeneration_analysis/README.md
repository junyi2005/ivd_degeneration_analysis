...
$$
ASI = \frac{|\text{SI}_{\text{peak-NP}} - \text{SI}_{\text{peak-AF}}|}{\text{SI}_{\text{mean-CSF}}}
$$
...
$$
ASI = \frac{\text{SI}_{\text{mean.NP}}}{\text{SI}_{\text{mean-CSF}}}
$$
...
1.  **构建Gabor滤波器组**: Gabor滤波器本质上是一个由高斯函数调制的复正弦函数。一个2D的Gabor滤波器可以通过以下公式定义：
    $$
    g(x, y; .ambda, .heta, .si, \sigma, \gamma) = .xp.eft(-\frac{x'^2 + \gamma^2 y'^2}{2\sigma^2}.ight) .os.eft(2.i \frac{x'}{.ambda} + .si.ight)
    $$
    其中：
...
3.  **中心矩 ($\mu_{pq}$)**: 为了消除平移对特征的影响，引入中心矩。它是在图像的质心 $(.ar{x}, .ar{y})$ 处计算的：
    $$
    \mu_{pq} = \sum_{x}\sum_{y} (x-.ar{x})^p (y-.ar{y})^q I(x, y) .uad \text{其中 } .ar{x} = \frac{M_{10}}{M_{00}}, .ar{y} = \frac{M_{01}}{M_{00}}
    $$
    其中，$M_{00}$ 是图像的总像素数或总质量。
4.  **归一化中心矩 ($.ta_{pq}$)**: 为了进一步消除缩放对特征的影响，中心矩需要进行归一化：
    $$
    .ta_{pq} = \frac{\mu_{pq}}{\mu_{00}^{(1 + \frac{p+q}{2})}}
    $$
...
2.  **LBP值**: 将这个二进制数转换为十进制数，即为中心像素的LBP值。本项目采用`uniform`模式，它将某些不常见的LBP模式映射到特定的值，并为所有非`uniform`模式分配一个公共值。
    $$
    LBP_{P,R}(x_c, y_c) = \sum_{p=0}^{P-1} s(g_p - g_c) 2^p .uad \text{其中 } s(z) = .egin{cases} 1 & z .eq 0 . 0 & z < 0 .nd{cases}
    $$
3.  **LBP特征**:
...
1.  **距离变换**
    ...
    *   **提取的特征**:
        *   **平均距离**: 整个ROI内所有前景像素距离变换值的平均值。它反映了形状的平均“厚度”或“饱满度”。
            $$
            \mu_{dist} = \frac{1}{|F|} \sum_{p .n F} EDT(p)
            $$
            其中 $|F|$ 是前景像素的总数。
...
2.  **骨架分析**
    ...
    *   **提取的特征**:
        ...
        $$
        C(p) = .eft( \sum_{i=-1}^{1} \sum_{j=-1}^{1} S(p_x+i, p_y+j) .ight) - 1
        $$
        *   **骨架像素数**: 构成骨架的总像素数量，反映了形状的整体“长度”或“范围”。
            $$
            N_{skel} = \sum_{p .n S} 1
            $$
        *   **分支点数**: 骨架上一个点连接了三条或更多路径的点，其数量反映了形状的复杂分支结构。一个像素 `p` 是分支点，当且仅当 $C(p) > 2$。
            $$
            N_{branch} = \sum_{p .n S} .athbb{I}(C(p) > 2) .uad \text{其中 } .athbb{I}(.dot) \text{ 是指示函数}
            $$
        *   **端点数**: 骨架上只连接了一条路径的点，反映了形状末梢的数量。一个像素 `p` 是端点，当且仅当 $C(p) = 1$。
            $$
            N_{end} = \sum_{p .n S} .athbb{I}(C(p) = 1)
            $$
...
*   **B样条插值算法**:
    ...
    *   **基本情况 (0次)**:
        $$
        N_{i,0}(t) = .egin{cases} 1 & \text{if } u_i \le t < u_{i+1} . 0 & \text{otherwise} .nd{cases}
        $$
    *   **递归步骤 ($p>0$)**:
        $$
        N_{i,p}(t) = \frac{t - u_i}{u_{i+p} - u_i} N_{i,p-1}(t) + \frac{u_{i+p+1} - t}{u_{i+p+1} - u_{i+1}} N_{i+1,p-1}(t)
        $$
    这里的 $u_i$ 是节点向量 $U = \mu_0, u_1, ..., u_m.$ 中的元素。节点向量是一个非递减的参数值序列，它决定了多项式段之间的连接点和曲线的平滑度。

    在 DSCR 的计算中，我们的目标是找到一条穿过所有给定地标点 $Q_j$ 的平滑曲线。这是一个插值问题：我们需要求解未知的控制点 $P_i$。通过为每个地标点 $Q_j$ 分配一个参数值 $t_j$，我们可以建立一个线性方程组：
    $$
    Q_j = C(t_j) = \sum_{i=0}^{n} P_i N_{i,p}(t_j) .uad \text{for } j=0, .ots, n
    $$
...
*   **关键特征**:
    ...
    *   **熵**: $\text{Entropy} = -\sum_{i=1}^{N_g} p(i) .og_2(p(i))$ (衡量图像强度的随机性或复杂性)
    *   **均值**, **标准差**, **偏度**, **峰度** 等标准统计量。
...
- 张量：花体大写字母（例如 $\mathcal{X}$），$N$ 阶张量为
  $$
  \mathcal{X}.n.athbb{R}^{I_1\times I_2\times.dots\times I_N}.
  $$
  向量是 1 阶张量，矩阵是 2 阶张量。

**模‑$n$ 展开**

张量的模-$n$ 展开是将高维张量重新排列为矩阵的过程。对于 $N$ 阶张量 $\mathcal{X} .n .athbb{R}^{I_1 \times I_2 \times .dots \times I_N}$，其模-$n$ 展开记为 $.athbf{X}_{(n)}$。
...
$$
j = 1 + \sum_{k=1, k .eq n}^{N} (i_k - 1) J_k, .uad \text{with } J_k = .rod_{m=1, m .eq n}^{k-1} I_m.
$$
直观上，$.athbf{X}_{(1)}$ 是将张量的“列”作为列向量排列，$.athbf{X}_{(2)}$ 是将张量的“行”作为列向量排列，$.athbf{X}_{(3)}$ 是将张量的“管” 作为列向量排列。

**模‑$n$ 张量–矩阵乘积**

$N$ 阶张量 $\mathcal{X} .n .athbb{R}^{I_1 \times I_2 \times .dots \times I_N}$ 与矩阵 $.athbf{U} .n .athbb{R}^{J \times I_n}$ 的模-$n$ 乘积记为 $\mathcal{X} \times_n .athbf{U}$，结果为一个 $N$ 阶张量，其尺寸为 $I_1 \times .dots \times I_{n-1} \times J \times I_{n+1} \times .dots \times I_N$。

定义
$$
(\mathcal{X} \times_n .athbf{U})_{i_1 .ots i_{n-1} j i_{n+1} .ots i_N} = \sum_{i_n=1}^{I_n} x_{i_1 .ots i_n .ots i_N} u_{j i_n}
$$

这等价于
$$
\mathcal{Y} = \mathcal{X} \times_n .athbf{U} .ff .athbf{Y}_{(n)} = .athbf{U} .athbf{X}_{(n)}
$$
...
$$
\mathcal{X}._F
=.qrt{\sum_{i_1,.ots,i_N} x_{i_1.dots i_N}^2},
$$
...
Tucker 分解可被视为高阶的主成分分析（PCA）。对于三阶张量 $\mathcal{X} .n .athbb{R}^{I_1 \times I_2 \times I_3}$，其分解形式为：
$$
\mathcal{X} .pprox \mathcal{G} \times_1 .athbf{U}^{(1)} \times_2 .athbf{U}^{(2)} \times_3 .athbf{U}^{(3)}
$$
其中：
- $\mathcal{G} .n .athbb{R}^{R_1 \times R_2 \times R_3}$ 为 Core Tensor，描述了不同模态因子之间的相互作用水平，其元素值反映了对应特征组合的能量或重要性。
...
$$
\mathcal{X}.n.athbb{R}^{I_1\times I_2\times I_3},
$$
CP 分解写作
$$
.at{\mathcal{X}}
=\sum_{r=1}^R .ambda_r. a_r.irc b_r.irc c_r,
$$
其中
...
C=[c_1,.ots,c_R],
$$
则 $.at{\mathcal{X}}$ 完全由 $(A,B,C,.ambda_1,.ots,.ambda_R)$ 决定。
...
对单个椎间盘 ROI $\mathcal{X}.n.athbb{R}^{I_x\times I_y\times I_z}$ 做 HOSVD / Tucker 分解。该过程旨在将高维 MRI 信号投影到由数据驱动的正交基底上，捕获其在三个空间方向上的“主变形模式”和整体结构复杂度：
...
3. 为获得低秩近似，需选定多线性截断秩 $(R_1,R_2,R_3)$。对每个模 $n=1,2,3$，令奇异值为
   $\sigma^{(n)}_k._{k\le 1}$，选最小 $R_n$ 使得
   $$
   \frac{\sum_{k=1}^{R_n}.igl(\sigma^{(n)}_k.igr)^2}
        {\sum_k .igl(\sigma^{(n)}_k.igr)^2}
   \le .ta,\quad .ta.pprox 0.95.
   $$
4. 构造截断因子矩阵
   $$
   .ilde{U}^{(n)} = U^{(n)}(:,1:R_n),
...
1. 各模主奇异值向量
   ...
   $$
   f^{(n)}_{\sigma}
   =.igl[\sigma^{(n)}_1,.ots,\sigma^{(n)}_{K_n}.igr].
   $$
   这些值反映该模整体能量及其衰减速度。
...
2. 各模能量分布
   ...
   $$
   e^{(n)}_k
   =\frac{.igl(\sigma^{(n)}_k.igr)^2}
          {\sum_j .igl(\sigma^{(n)}_j.igr)^2},
   .uad k=1,.ots,K_n,
   $$
   刻画能量是否集中在少数主方向上。
...
3. core tensor 能量与方向“重要性”分布
   ...
   - 沿第 1 模的能量分布：
     ...
     =\frac{\sum_{j,k}.ilde{\mathcal{S}}_{ijk}^2}
            {..ilde{\mathcal{S}}._F^2},.uad i=1,.ots,R_1,
     $$
     类似可定义 $p_2(j),p_3(k)$。
...
4. 重构误差比例
   ...
   $$
   r
   =\frac{\mathcal{X}-.at{\mathcal{X}}._F^2}
          {\mathcal{X}._F^2}.
   $$
...
$$
\mathcal{Y} = f_{VST}(\mathcal{Y}_{raw}, \sigma_n) = .qrt{\mathcal{Y}_{raw}^2 + \sigma_{n}^2}
$$
...
$$
\mathcal{Y}^{(k)} = \mathcal{X}^{(k-1)} + .lpha (\mathcal{Y} - \mathcal{X}^{(k-1)})
$$
其中 $\mathcal{Y}$ 是初始 VST 数据，$\mathcal{X}^{(k-1)}$ 是上一轮的恢复结果（初始 $\mathcal{X}^{(0)} = \mathcal{Y}$），$.lpha$ 为反馈系数。
...
- 堆叠：将这 $n$ 个相似块沿第 4 维堆叠，形成观测张量 $\mathcal{G}_{\mathcal{Y}} .n .athbb{R}^{m \times m \times m \times n}$。

##### 1.4.4.3 对数和正则化模型 (NLRTA-LSR)

对每个 patch 组张量 $\mathcal{G}_{\mathcal{Y}}$，旨在寻找低秩近似张量 $\mathcal{G}_{\mathcal{X}}$，优化目标函数定义为：
$$
\in_{\mathcal{G}_{\mathcal{X}}} \sum_{i=1}^{4} .heta_i .igl\mathcal{G}_{\mathcal{X},(i)}.igr._{log} .uad \text{s.t.} .uad \mathcal{G}_{\mathcal{Y}} - \mathcal{G}_{\mathcal{X}}._F^2 \le .arepsilon
$$
转化为无约束拉格朗日形式：
$$
\in_{\mathcal{G}_{\mathcal{X}}} \sum_{i=1}^{4} .heta_i .igl\mathcal{G}_{\mathcal{X},(i)}.igr._{log} + \frac{.u}{2} \mathcal{G}_{\mathcal{Y}} - \mathcal{G}_{\mathcal{X}}._F^2
$$

其中：
- $\mathcal{G}_{\mathcal{X},(i)}$ 是张量沿第 $i$ 模的展开矩阵；
- $.heta_i$ 为权重，满足 $.um .heta_i = 1$且 $.heta_i \le 0$；
- $..athbf{A}._{log}$ 为矩阵的**对数和范数**，定义为：
  $$
  ..athbf{A}._{log} = \sum_{j=1}^{r} .og(.elta_j(.athbf{A}) + .arepsilon)
  $$
...
1. 更新辅助变量 $\mathcal{O}_i$ (转化为 WNNM 问题)：
   ...
   $$
   \mathcal{O}_i^{t+1} = .rg\in_{\mathcal{O}_i} \frac{.heta_i}{.u^t} \mathcal{O}_{i,(i)}._{log} + \frac{1}{2} \mathcal{O}_i - \mathcal{T}_i^t._F^2
   $$
   其中:
   - $\mathcal{T}_i^t = \mathcal{G}_{\mathcal{X}}^t - \frac{\mathcal{Q}_i^t}{.u^t}$ ($\mathcal{Q}_i$ 为拉格朗日乘子)。
   - $\mathcal{O}_i^{t+1}$：本轮迭代中，基于第 $i$ 模态展开矩阵奇异值收缩后折叠回来的张量。
   - $\mathcal{Q}_i^t$：第 $i$ 个约束对应的拉格朗日乘子张量（记录了上一轮的残差信息）。
   - $.u^t$：当前的惩罚参数。
   
   根据 Wang, Z., et al. (2022) Lemma 1，该问题有闭式解，通过加权奇异值阈值算子(WSVT)求解。设 $\mathcal{T}_{i,(i)}$ 的奇异值分解为 $.athbf{U}.athbf{\sigma}.athbf{V}^T$，则更新后的奇异值 $\pi_j$ 为：
   $$
   \pi_j(\mathcal{O}_{i,(i)}) = .ax.eft(0, \frac{c_1 + .qrt{c_2}}{2}.ight) .uad \text{if } c_2 \le 0, \text{ else } 0
   $$
   其中：
   - $c_1 = .elta_j(\mathcal{T}_{i,(i)}) - .arepsilon$
   - $c_2 = (.elta_j(\mathcal{T}_{i,(i)}) + .arepsilon)^2 - \frac{4.heta_i}{.u^t}$
   
   **注意**：此处产生的自适应权重隐含在阈值操作中，其等效权重 $.mega_j$ 与奇异值大小成反比，即 $.mega_j = \frac{1}{.elta_j + .arepsilon}$。

2. 更新全局张量 $\mathcal{G}_{\mathcal{X}}$：
   $$
   \mathcal{G}_{\mathcal{X}}^{t+1} = \frac{1}{4} \sum_{i=1}^{4} .eft( \mathcal{O}_i^{t+1} + \frac{\mathcal{Q}_i^t}{.u^t} .ight)
   $$

3. 更新乘子 $\mathcal{Q}_i$ 与惩罚参数 $.u$：
   $$
...
利用上述算法对每个 patch 组优化后的低秩张量 $.at{\mathcal{X}}_{group}$ 及其过程参数提取特征：

1. 各模态的奇异值

   对最终恢复的低秩张量 $.at{\mathcal{X}}_{group}$ 进行四个方向的模态展开，得到矩阵 $.at{\mathcal{X}}_{(n)}$ ($n=1,2,3,4$)。对每个矩阵进行 SVD 分解，获取奇异值向量：
   $$
...
2. 自适应权重向量
   ...
   $$
   .mega_j^{(n)} = \frac{1}{.elta_j^{(n)} + .arepsilon}
   $$
...
3. 对数和正则化强度
   ...
   $$
   L^{(i)} = \sum_{j} .og(.elta_j^{(i)} + .arepsilon)
   $$
...
4. 非局部自相似残差
   ...
   $$
   e_{NL} = \frac{\mathcal{G}_{\mathcal{Y}} - .at{\mathcal{G}}_{\mathcal{X}}._F^2}{\mathcal{G}_{\mathcal{Y}}._F^2}
   $$
   同时计算所有 patch 组 $e_{NL}$ 的均值和标准差，反映该区域是否符合“非局部低秩”假设，病变区域往往具有异常的结构复杂性。

5. 对数和有效秩

   对最终低秩张量 $.at{\mathcal{G}}_{\mathcal{X}}$ 的 4 个模态展开矩阵，分别计算：
   $$
   R_{log}^{(i)} = \sum_{j} .og.eft(.elta_j(.at{\mathcal{G}}_{\mathcal{X},(i)}) + .arepsilon.ight), .uad i=1,2,3,4
   $$
...
$$
\mathcal{X}.n.athbb{R}^{I_x\times I_y\times I_z},
$$
考虑秩为 $R$ 的 CP 分解：
$$
.at{\mathcal{X}}
=\sum_{r=1}^{R}.ambda_r.dot.athbf{a}_r.irc.athbf{b}_r.irc.athbf{c}_r,
$$
其中：
...
1. 初始化（直接用 HOSVD 结果初始化）
    ...
    - 初始权重：
      $$
      .ambda_r^{(0)}
      =.angle\mathcal{X},.athbf{a}_r^{(0)}.irc.athbf{b}_r^{(0)}.irc.athbf{c}_r^{(0)}.angle
      =\sum_{i,j,k}x_{ijk}.athbf{a}^{(0)}_{r,i}.athbf{b}^{(0)}_{r,j}.athbf{c}^{(0)}_{r,k}.
      $$
2. ALS 迭代更新因子矩阵
   ...
   $$
   .athbf{C}.st.athbf{B}
   =[.athbf{c}_1\times.athbf{b}_1,.ots,.athbf{c}_R\times.athbf{b}_R]
...
3. 列归一化与权重更新
   ...
   为保持稳定性和可解释性，在每轮迭代末，对每个成分 $r$：
    ...
    - 归一化列向量：
      $$
      .athbf{a}_r^{(t+1)}.eftarrow\frac{.athbf{a}_r^{(t+1)}}{s_a},.uad
      .athbf{b}_r^{(t+1)}.eftarrow\frac{.athbf{b}_r^{(t+1)}}{s_b},.uad
      .athbf{c}_r^{(t+1)}.eftarrow\frac{.athbf{c}_r^{(t+1)}}{s_c};
      $$
    - 将缩放系数累积到权重：
      $$
...
4. 收敛条件
   ...
   $$
   .at{\mathcal{X}}^{(t)}
   =\sum_{r=1}^R.ambda_r^{(t)}.athbf{a}_r^{(t)}.irc.athbf{b}_r^{(t)}.irc.athbf{c}_r^{(t)},.uad
   \mathcal{E}^{(t)}=\mathcal{X}-.at{\mathcal{X}}^{(t)}.
   $$
...
$$
r_{\text{CP}}
=\frac{\mathcal{X}-.at{\mathcal{X}}._F^2}{\mathcal{X}._F^2},
$$
...
1. 权重与能量谱特征
   ...
   - 相对能量比例：
      $$
      E_{.ambda}
      =\sum_{r=1}^R .ilde{.ambda}_r^2,\quad
      e_r
      =\frac{.ilde{.ambda}r^2}{E_{.ambda}+.arepsilon},.r=1,.ots,R,
      $$
      其中 $.arepsilon$ 防止除零。向量 $f_e=[e_1,.ots,e_R]^T$ 描述能量是否集中在少数几个成分。
   - 有效 CP 秩：
      $$
      R_{\text{eff}}
      =\frac{.eft(\sum_{r=1}^R .ilde{.ambda}_r^2.ight)^2}
      {\sum_{r=1}^R .ilde{.ambda}_r^4+.arepsilon}.
      $$
...
2. 因子向量的“集中度”与熵
   ...
   $$
   p^{(a)}_{r,i}
   =\frac{.igl(.ilde{.athbf{a}}_{r,i}.igr)^2}{\sum_{k=1}^{I_x}.igl(.ilde{.athbf{a}}_{r,k}.igr)^2},
   .uad i=1,.ots,I_x,
   $$
   ...
   - 熵：
      $$
      H^{(a)}_r
      =-\sum_{i=1}^{I_x}p^{(a)}_{r,i}.og(p^{(a)}_{r,i}+.arepsilon),
      $$
      $$
   ...
   - 集中度（类似 Gini 指标）：
      $$
      G^{(a)}_r
      =\sum_{i=1}^{I_x}.igl(p^{(a)}_{r,i}.igr)^2,
      $$
      $$
...
3. 整体强度
   ...
   $$
   S_{.ambda}
   =\sum_{r=1}^R|.ilde{.ambda}_r|,\quad
   ..oldsymbol{.ilde{.ambda}}._2
   =.eft(\sum_{r=1}^R.ilde{.ambda}_r^2.ight)^{1/2}.
   $$
...
| `energy_threshold` | `0.95` | 能量阈值 $.ta$。对每个模的奇异值序列，选择最小的多线性秩 $R_n$，使前 $R_n$ 个奇异值的能量比例 $\sum_{k=1}^{R_n}\sigma_k^2 / \sum_j \sigma_j^2 \le .ta$。该值越接近 1，保留的主模态越多，重构误差越小，但特征维度增加。 |
...
| `beta_noise` | `0.3` | 噪声估计参数 $\eta$，用于在 ROI 中估计 Rician 噪声标准差 $\sigma_n$。 |
...
*   **原理**:
    ...
    3.  **执行随机形态学操作**: 使用上一步随机生成的参数，对掩码执行选定的膨胀或腐蚀操作。
        $$
        \text{Mask}_{\text{perturbed}} = .egin{cases} \text{Mask}_{\text{original}} .plus B_{\text{rand}} & \text{if } p > 0.5 . \text{Mask}_{\text{original}} \minus B_{\text{rand}} & \text{if } p \le 0.5 .nd{cases}
        $$
...
*   **原理与算法**:
    ...
    2.  **定义高斯分布**:
        该高斯分布的概率密度函数由以下公式定义：
        $$
        P(z) = \frac{1}{\sigma .qrt{2.i}} e^{ - \frac{(z - .u)^2}{2\sigma^2} }
        $$
        根据代码实现，该分布的参数被设定为：
        *   **均值 $.u$**: 固定为 0。
...
#### I. 组内相关系数
  ...
  * **计算原理**:
    ...
    3.  **计算离差平方和**:
        *   **总离差平方和**: 反映数据的总变异。
            $$
            SS_{\text{总}} = \sum_{i=1}^{n} \sum_{j=1}^{k} (x_{ij} - .ar{x})^2
            $$
        *   **行间离差平方和 (病例间)**: 反映由不同病例引起的变异。
            $$
            SS_{\text{行}} = k \sum_{i=1}^{n} (.ar{x}_{i.} - .ar{x})^2
            $$
        *   **列间离差平方和 (条件间)**: 反映由不同扰动条件引起的变异。
            $$
            SS_{\text{列}} = n \sum_{j=1}^{k} (.ar{x}_{.j} - .ar{x})^2
            $$
...
#### II. 分层聚类
  ...
  * **原理与算法**:
    ...
    2.  **距离矩阵计算**: 使用欧几里得距离计算每两个特征向量之间的距离，形成一个距离矩阵。向量 $.athbf{a}$ 和 $.athbf{b}$ 之间的距离定义为：
        $$
        d(.athbf{a}, .athbf{b}) = .qrt{\sum_{i=1}^{k} (a_i - b_i)^2}
        $$
    ...
    3.  **Ward链接算法**:
        ...
        *   **迭代合并**: 在每一步中，算法会计算所有可能的簇对合并后，所导致的总簇内误差平方和的增量。
            *   单个簇 $C_m$ 的误差平方和定义为簇内所有特征向量到该簇质心（均值向量）$.athbf{.u}_m$ 的平方距离之和：
                $$
                E_m = \sum_{.athbf{f}_i .n C_m} ||.athbf{f}_i - .athbf{.u}_m||^2
                $$
...
#### III. Spearman相关性分析
  ...
  * **原理与算法**:
    ...
    1.  **计算相关系数**:
        ...
        $$
        .ho = \frac{\sum_{i=1}^{n}(rg_{X_i} - .verline{rg_X})(rg_{Y_i} - .verline{rg_Y})}{.qrt{\sum_{i=1}^{n}(rg_{X_i} - .verline{rg_X})^2 \sum_{i=1}^{n}(rg_{Y_i} - .verline{rg_Y})^2}}
        $$
