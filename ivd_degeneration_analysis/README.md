...
* **椎体高度 (  $\text{Height}_{\text{lower}}$ )**: 对于椎间盘上、下方的椎体，程序通过识别其各自掩码的像素区域，并计算这些像素在图像垂直方向（通常是 $y$ 轴）上的最大范围。这个垂直范围乘以 $y$ 方向的体素间距，即得到椎体的高度。还有 （ $\text{Height}_{\text{upper}}$ ）
...
*  $\text{SI}_{\text{peak-NP}}$ ， $\text{SI}_{\text{peak-AF}}$ **:
...
$$
 ...
 其中：
 * $x' = x \cos\theta + y \in\theta $ 和 $ y' = -x \in\theta + y \cos\theta$ 是旋转后的坐标。
...
$$
LBP_{P,R}(x_c, y_c) = \sum_{p=0}^{P-1} s(g_p - g_c) 2^p \quad \text{其中 } s(z) = \begin{cases} 1 & z \leq 0 . 0 & z < 0 \end{cases}
$$
...
$$
EDT(p) = \in_{q .n B} \sqrt{(p_x - q_x)^2 + (p_y - q_y)^2}
$$
...
$$
N_{branch} = \sum_{p .n S} \mathbb{I}(C(p) > 2) \quad \text{其中 } \mathbb{I}(\cdot) \text{ 是指示函数}
$$

 * **端点数**: 骨架上只连接了一条路径的点，反映了形状末梢的数量。一个像素 `p` 是端点，当且仅当 $C(p) = 1$ 。

$$
N_{end} = \sum_{p .n S} \mathbb{I}(C(p) = 1)
$$
...
- 标量：小写字母（例如 $ x $ ）。
- 向量：粗体小写字母（例如 $ \mathbf{x} $ ）， $ \mathbf{x}.n\mathbb{R}^I $ 。
- 矩阵：粗体大写字母（例如 $ \mathbf{X} $ ）， $ \mathbf{X}.n\mathbb{R}^{I\times J} $ 。
...
$$
 ...
 \mathcal{X}.n\mathbb{R}^{I_1\times I_2\times\dots\times I_N}.
...
张量的模- $ n$ 展开是将高维张量重新排列为矩阵的过程。对于 $ N $ 阶张量 $ \mathcal{X} .n \mathbb{R}^{I_1 \times I_2 \times \dots \times I_N} $ ，其模- $ n$ 展开记为 $ \mathbf{X}_{(n)} $ 。

定义： $ \mathbf{X}_{(n)} .n \mathbb{R}^{I_n \times (I_1 \dots I_{n-1} I_{n+1} \dots I_N)} $ 的列向量由张量的模- $ n$ fibers 组成。具体地，张量元素 $ (i_1, i_2, \dots, i_N) $ 映射到矩阵元素 $ (i_n, j) $ ，其中

$$

j = 1 + \sum_{k=1, k \leq n}^{N} (i_k - 1) J_k, \quad \text{with } J_k = .rod_{m=1, m \leq n}^{k-1} I_m.
...
$ N $ 阶张量 $ \mathcal{X} .n \mathbb{R}^{I_1 \times I_2 \times \dots \times I_N} $ 与矩阵 $ \mathbf{U} .n \mathbb{R}^{J \times I_n} $ 的模- $ n$ 乘积记为 $ \mathcal{X} \times_n \mathbf{U} $ ，结果为一个 $ N $ 阶张量，其尺寸为 $ I_1 \times \dots \times I_{n-1} \times J \times I_{n+1} \times \dots \times I_N $ 。
...
\mathcal{X}._F
=\sqrt{\sum_{i_1,\dots,i_N} x_{i_1\dots i_N}^2},
...
Tucker 分解可被视为高阶的主成分分析（PCA）。对于三阶张量 $ \mathcal{X} .n \mathbb{R}^{I_1 \times I_2 \times I_3} $ ，其分解形式为：
...
其中：
- $ \mathcal{G} .n \mathbb{R}^{R_1 \times R_2 \times R_3} $ 为 Core Tensor，描述了不同模态因子之间的相互作用水平，其元素值反映了对应特征组合的能量或重要性。
- $ \mathbf{U}^{(n)} .n \mathbb{R}^{I_n \times R_n} $ 为正交的因子矩阵，通常包含该模态下的主成分基向量。
...
\mathcal{X}.n\mathbb{R}^{I_1\times I_2\times I_3},
...
其中
- $ \lambda_r.n\mathbb{R},.a_r.n\mathbb{R}^{I_1},.b_r.n\mathbb{R}^{I_2},.c_r.n\mathbb{R}^{I_3} $ ；
- $ \circ $ 为外积运算；
- $ R $ 为 CP rank。
...
对于标量 $ x .n \mathbb{R} $ 和阈值 $ \tau > 0 $ ，软阈值算子定义为：
...
对单个椎间盘 ROI $ \mathcal{X}.n\mathbb{R}^{I_x\times I_y\times I_z} $ 做 HOSVD / Tucker 分解。该过程旨在将高维 MRI 信号投影到由数据驱动的正交基底上，捕获其在三个空间方向上的“主变形模式”和整体结构复杂度：
...
1. 各模主奇异值向量
 ...
 取每个模的前 $ K_n $ 个奇异值， $ K_n\approx 10\lim 15 $ ：
...
- 堆叠：将这 $ n $ 个相似块沿第 4 维堆叠，形成观测张量 $ \mathcal{G}_{\mathcal{Y}} .n \mathbb{R}^{m \times m \times m \times n} $ 。
...
\in_{\mathcal{G}_{\mathcal{X}}} \sum_{i=1}^{4} \theta_i \bigl\mathcal{G}_{\mathcal{X},(i)}\bigr._{log} \quad \text{s.t.} \quad \mathcal{G}_{\mathcal{Y}} - \mathcal{G}_{\mathcal{X}}._F^2 \le \varepsilon
...
\in_{\mathcal{G}_{\mathcal{X}}} \sum_{i=1}^{4} \theta_i \bigl\mathcal{G}_{\mathcal{X},(i)}\bigr._{log} + \frac{.u}{2} \mathcal{G}_{\mathcal{Y}} - \mathcal{G}_{\mathcal{X}}._F^2
...
- $ \theta_i $ 为权重，满足 $ \sum \theta_i = 1 $ 且 $ \theta_i \le 0 $ ；
...
$$
 ...
 - $ c_2 = (\delta_j(\mathcal{T}_{i,(i)}) + \varepsilon)^2 - \frac{4\theta_i}{.u^t} $
...
\mathcal{X}.n\mathbb{R}^{I_x\times I_y\times I_z},
...
- $ \lambda_r.n\mathbb{R} $ 为第 $ r $ 个秩‑1 成分的权重；
- $ \mathbf{a}_r.n\mathbb{R}^{I_x} $ 、$\mathbf{b}_r.n\mathbb{R}^{I_y} $ 、$\mathbf{c}_r.n\mathbb{R}^{I_z} $ 分别为三条空间方向上的因子向量；
- $ \circ $ 为向量外积。
...
\mathbf{A}=[\mathbf{a}_1,\dots,\mathbf{a}_R].n\mathbb{R}^{I_x\times R},\quad
\mathbf{B}=[\mathbf{b}_1,\dots,\mathbf{b}_R].n\mathbb{R}^{I_y\times R},\quad
\mathbf{C}=[\mathbf{c}_1,\dots,\mathbf{c}_R].n\mathbb{R}^{I_z\times R}.
...
\in_{\mathbf{A},\mathbf{B},\mathbf{C},\boldsymbol{\lambda}}
\left.
\mathcal{X}
-\sum_{r=1}^R \lambda_r\cdot \mathbf{a}_r\circ \mathbf{b}_r\circ \mathbf{c}_r
\right._F^2.
...
若在绝大多数 ROI 上 $ r_{\text{CP}}<0.05\lim 0.1 $ ，说明 $ R $ 足以表达主要结构。
...
| `energy_threshold` | `0.95` | 能量阈值 $ \eta $ 。对每个模的奇异值序列，选择最小的多线性秩 $ R_n $ ，使前 $ R_n $ 个奇异值的能量比例 $ \sum_{k=1}^{R_n}\sigma_k^2 / \sum_j \sigma_j^2 \le \eta $ 。该值越接近 1，保留的主模态越多，重构误差越小，但特征维度增加。 |
 ...
 * 单个簇 $ C_m $ 的误差平方和定义为簇内所有特征向量到该簇质心（均值向量） $ \mathbf{.u}_m $ 的平方距离之和：
